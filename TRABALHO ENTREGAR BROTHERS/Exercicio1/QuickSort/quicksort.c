...
//----------------------------------------------------
int particao(Intervalo *v, int e, int d)
{
	int i, j; 
	Intervalo aux;
	i = e;
	for (j = e + 1; j <= d; j++) {
		if (v[j].inicio < v[e].inicio || (v[j].inicio == v[e].inicio && v[j].fim < v[e].fim)) {
			i++;
			aux.inicio = v[i].inicio;
			aux.fim = v[i].fim;
			v[i].inicio = v[j].inicio;
			v[i].fim = v[j].fim;
			v[j].inicio = aux.inicio;
			v[j].fim = aux.fim;
		}
	}
	aux.inicio = v[e].inicio;
	aux.fim = v[e].fim;
	v[e].inicio = v[i].inicio;
	v[e].fim = v[i].fim;
	v[i].inicio = aux.inicio;
	v[i].fim = aux.fim;
			
	return i;
}

void sort(Intervalo *vetor, int p, int r){
	int q;
	if (p < r) {
