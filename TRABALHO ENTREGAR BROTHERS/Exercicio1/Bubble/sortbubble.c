...
void sort(Intervalo *vetor){
	...
	for (i = N - 1; i > 0; i--) {
		for (j = 0; j < i; j++) {
			if (vetor[j].inicio > vetor[j + 1].inicio) {
				aux.inicio = vetor[j].inicio;
				aux.fim = vetor[j].fim;
				vetor[j].inicio = vetor[j + 1].inicio;
				vetor[j].fim = vetor[j + 1].fim;
				vetor[j + 1].inicio = aux.inicio;
				vetor[j + 1].fim = aux.fim;
			}else if((vetor[j].inicio == vetor[j + 1].inicio) && (vetor[j].fim > vetor[j + 1].fim)){
                aux.inicio = vetor[j].inicio;
				aux.fim = vetor[j].fim;
				vetor[j].inicio = vetor[j + 1].inicio;
				vetor[j].fim = vetor[j + 1].fim;
